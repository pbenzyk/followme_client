...
class AuthModule extends VuexModule {
  ...
  public async setUser(){
    let user = await Core.getHttp(`/api/v1/auth/user/`)
    this.user = user;
    return user;
  }
  ...
  public async register(form:any){
    await this.reToken();``
    let user = await Core.postHttp(`/api/v2/auth/register/`, form)
    if(user.id){
        return user;  
    }else{
      return false
  ...
  public async login(form:any){
    await this.reToken();``
    let user = await Core.postHttp(`/api/v1/auth/login/`, form)
    if(user.key){
      let key = user.key
      console.log(key);
  ...
  public async logout(){
    localStorage.clear();
    return await Core.postHttp('/api/v1/auth/logout/',{})
  }
