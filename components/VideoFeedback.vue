<template>
  <div>
    <video
      ...
      @timeupdate="
        $refs.videoObj ? $emit('timeupdate', $refs.videoObj.currentTime) : ''
      "
      @ended="$emit('ended')"
    ></video>
    <canvas
  ...
  methods: {
    ...
    setCurrentTime(time) {
      ...
      if (this.poseVideoJson) {
        ...
        while (
          this.poseVideoJson.recorded[this.recordedVideoIndex] &&
          time > this.poseVideoJson.recorded[this.recordedVideoIndex].time
        ) {
    ...
    async poseDetectionFrame(force = false) {
      if (
        !this.$refs.videoObj ||
        !this.$refs.videoOut ||
        (!force && this.$refs.videoObj.paused)
      )
        return;
      ...
      try {
        ...
        if (this.type === "learner") {
          Feedback.getFeedback(
            this.videoOptions.feedbackType,
            state.pose["trainer"],
            state.pose["learner"],
            this.videoOptions.drawFeedbackBody,
            this.videoOptions.drawFeedbackArms,
            this.videoOptions.drawFeedbackLegs,
        ...
        ctxVideo.clearRect(
          0,
          0,
          this.$refs.videoOut.width,
          this.$refs.videoOut.height
        );
        ctxVideo.drawImage(
    ...
    async poseDetectionVideo() {
      ...
      if (
        ...
        while (
          this.poseVideoJson.recorded[this.recordedVideoIndex] &&
          this.timestamp >
            this.poseVideoJson.recorded[this.recordedVideoIndex].time
        ) {
          this.recordedVideoIndex++;
        }
        ...
        // Realtime estimation
        var posesVideo = await this.detector.estimatePoses(
          this.offscreenCanvas
        );
